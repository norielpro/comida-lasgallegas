...
/*function saludarCliente() {
    ...
    const tardenoche = moment().set({ hour: 20, minute: 0 }); // 05:00 PM
...
function m() {
   ...
   const abrir = Number(datos.abrir)
    const cerrar = Number (datos.cerrar)  
       ...
       // Define el horario en el que quieres mostrar el botón
    const horaInicio = moment().set({ hour: abrir, minute: 0 }); // 09:00 AM
    const horaFin = moment().set({ hour: cerrar, minute: 0 }); // 05:00 PM
...
function enviarFactura(){
 document.getElementById("reciboFactura").style.display= 'flex'
   ...
   localStorage.setItem("facturasPendientes",[])
    ...
    botonWa.href = `https://api.whatsapp.com/send?phone=${numTel}&text=${textoFinal} `
